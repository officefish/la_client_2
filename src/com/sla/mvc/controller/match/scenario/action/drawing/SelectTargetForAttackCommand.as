package com.sla.mvc.controller.match.scenario.action.drawing {
	 ...
	 * @author inozemcev
	 */
	public class SelectTargetForAttackCommand extends StarlingCommand 
	{
		[Inject]
		...
		private var iniciatorMinion:IMinion;
		private var serviceData:Dictionary;
		
		override public function execute():void 
		{
			MonsterDebugger.log ('InitAttackTagetCommand::execute()');
			...
			iniciatorMinion = event.data.minion;
			iniciatorMinion.block = true;
			iniciatorMinion.stopGlow();
			iniciatorMinion.dropShadow();

			field.stopGlowMinionsExceptMinion(iniciatorMinion);
			//MonsterDebugger.log (event.data.minion);
			iniciatorMinion.asDO().scaleX = 1.1;
		...
		private function selectMinion (data:Object) :void {
			var initiatorIndex:int = iniciatorMinion.asDO().parent.getChildIndex(iniciatorMinion.asDO());
			dispatch(new SceneEvent (SceneEvent.SELECT_FOR_ATTACK_INIT, { initiatorIndex:initiatorIndex, targetIndex:data.index } ));
		}
		
		private function cancelSelect() :void {
			iniciatorMinion.block = false;
			iniciatorMinion.stopDropShadow();
			field.glowMinions();
			iniciatorMinion.asDO().scaleX = iniciatorMinion.asDO().scaleY = 1; 
		}
